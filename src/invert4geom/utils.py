...
from polartoolkit import fetch, profiles
...
def create_topography(
    ...
    dataarray_to_add: xr.DataArray | None = None,
) -> xr.Dataset:
    """
    ...
    dataarray_to_add : xarray.DataArray | None, optional
        additional dataarray to add to the topography dataset, by default None
    ...
    if method == "flat":
        ...
        if len(df) == 1:
            ...
            if projection is not None:
                ...
                eastings, northings = zip(*projected_points)
                df["easting"] = eastings
                ...
                eastings, northings = zip(*projected_corners)
                cartesian_region = (
                    min(eastings),
                    max(eastings),
                    min(northings),
    ...
    if dataarray_to_add is not None:
        ds[dataarray_to_add.name] = dataarray_to_add
