...
* [fork](https://docs.github.com/en/get-started/exploring-projects-on-github/contributing-to-a-project) the [repository](https://github.com/mdtanker/invert4geom) using the `Fork` button on GitHub.
* clone your forked repository on your computer: `git clone https://github.com/mdtanker/invert4geom`.
...
* [make a Pull Request](http://makeapullrequest.com/) for your branch from the main GitHub repository [PR page](https://github.com/mdtanker/invert4geom/pulls).
...
```bash
git clone https://github.com/mdtanker/invert4geom
```
...
This contains your local, editable version of invert4geom, meaning if you alter code in the package, it will automatically include those changes in your environment (you may need to restart your kernel if using Jupyter). If you need to update the dependencies, see the [update the dependencies](#update-the-dependencies) section below.
...
When writing code; use logging (instead of print) to inform users of info, errors, and warnings. In each module `.py` file, import the project-wide logger instance with `from invert4geom import logger` and then for example: `logger.info("log this message")`
...
```
pixi run test tests/test_utils.py::test_name
```
...
[post on the GH discussions page](https://github.com/mdtanker/invert4geom/discussions).
...
If you just want to add a package to your environment temporarily, but not included it as a dependencies of `invert4geom`, use can use `pixi add PACKAGE`. The package will be added to the pixi portion of the pyproject.toml, but not to the packages dependencies. When a user installs `invert4geom`, they will only get the dependencies list in the [project][dependencies] section, not those listed in the [tool.pixi.dependencies] section.
